...
export function Logo({ light = false }: { light?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2.5">
      ...
      <img src="/img/logo-icon.svg" alt="" className="size-9" />
      <span className={`text-[22px] font-extrabold tracking-tight ${light ? "text-white" : "text-navy"}`}>중소나라</span>
    </Link>
  );
}
      ...
      <div className="container-x flex h-16 items-center gap-4">
        ...
        >
          <label className="flex h-11 max-w-xl items-center gap-2 rounded-full border border-line bg-bg px-4 focus-within:border-navy">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-ink-muted"><circle cx="11" cy="11" r="7" /><path d="m20 20-3.5-3.5" /></svg>
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="제품명, 브랜드, 모델명 검색" className="w-full bg-transparent text-sm outline-none placeholder:text-ink-muted" />
          </label>
        </form>
        <nav className="ml-auto flex items-center gap-2">
          <Link href="/quote" className="btn-orange hidden h-10 px-4 sm:inline-flex">견적요청</Link>
          <Link href="/cart" className="relative grid size-10 place-items-center rounded-full hover:bg-bg" aria-label="장바구니">
            ...
            {count > 0 && <span className="absolute -right-0.5 -top-0.5 grid min-w-5 place-items-center rounded-full bg-orange px-1 text-[11px] font-bold text-white">{count}</span>}
          </Link>
        </nav>
      </div>
