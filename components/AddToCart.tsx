...
export default function AddToCart({ p }: { p: Product }) {
  ...
  return (
    <div className="card sticky top-32 p-5">
      <div className="text-sm text-muted">단가 (VAT 별도)</div>
      <div className="text-3xl font-extrabold">{won(p.price)}</div>
      <div className="mt-1 text-xs text-muted">최소주문 {p.moq}개 · 납기 {p.lead}</div>
      ...
      <div className="mt-5 grid gap-2">
        ...
        <button onClick={() => { cart.add(p.handle, qty); router.push("/quote"); }} className="btn-accent w-full">이 수량으로 견적요청</button>
      </div>
      <p className="mt-3 text-[11px] leading-relaxed text-muted">{p.moq * 5}개 이상 주문 시 제조사 직접 협의 단가가 적용됩니다. 견적요청을 이용하세요.</p>
    </div>
  );
}
