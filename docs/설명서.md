...
│  └─ ui/               # shadcn/ui + Magic UI (button·badge·card·marquee·number-ticker…)
├─ lib/
...
### 3-3. Higgsfield 이미지 생성 (히어로·상품 사진)
...
3. 실행
   ...
   npm run gen:images
   ```
   - 생성물: `hero.jpg` · `brands.jpg` · 상품 20종 `.jpg` (로고는 SVG 고정, 생성 대상 아님)
   - 이미 있는 파일은 건너뜀 → 다시 만들려면 파일 삭제 후 재실행
   - 모델 변경: `HF_MODEL=...` (기본 `flux-pro/kontext/max/text-to-image`)
   - 동시 생성 수: `HF_CONC=3`
4. 프롬프트 수정: `lib/data.ts` → `heroPrompts` / 각 상품 `look`

### 3-4. 디자인 시스템·토큰
- 인트로(히어로)는 **shadcn/ui**(GitHub 114k★) + **Magic UI**(21k★) 컴포넌트: `components/ui/` (button·badge·card·dot-pattern·animated-shiny-text·number-ticker·border-beam·marquee). 추가는 `npx shadcn@latest add <name>` 또는 `"https://magicui.design/r/<name>"`.
- 로고: `public/img/logo-icon.svg`(원본) → `app/icon.png`(파비콘 512) · `app/apple-icon.png`(180) · `app/opengraph-image.png`(1200×630, 링크 공유 썸네일). SVG 수정 후 PNG는 헤드리스 Edge `--screenshot --default-background-color=00000000` 으로 재생성.

`app/globals.css` `:root`
| 토큰 | 값 | 용도 |
|---|---|---|
| `--navy` / `--primary` | `#12305b` | 주색(버튼·로고·강조). shadcn primary 도 네이비 |
| `--orange` | `#ff7a1a` | CTA(견적요청). 클래스 `bg-orange` `text-orange` `btn-orange` |
| `--ink-muted` | `#6b7280` | 보조 텍스트 `text-ink-muted` |
| `--bg` / `--surface` | `#f6f6f3` / `#fff` | 배경/카드 |
| 폰트 | Pretendard Variable | 본문·제목 전체 |

> ⚠️ `--accent` / `--muted` 는 shadcn 예약 토큰(호버 회색). 브랜드색에 쓰지 말 것.
