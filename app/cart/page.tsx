...
export default function CartPage() {
  ...
  return (
    <div className="container-x grid gap-8 py-8 md:grid-cols-[1fr_320px]">
      <div>
        <h1 className="mb-5 text-2xl font-bold">장바구니 <span className="text-base font-normal text-muted">{lines.length}개 품목</span></h1>
        <div className="card divide-y divide-line">
          {lines.map(({ p, qty }) => (
            <div key={p.handle} className="flex gap-4 p-4">
              <ProductImage p={p} className="size-20 shrink-0 rounded-lg" />
              <div className="min-w-0 flex-1">
                <div className="text-xs text-muted">{brandOf(p).name}</div>
                <Link href={`/products/${p.handle}`} className="line-clamp-1 font-semibold hover:text-navy">{p.title}</Link>
                <div className="mt-0.5 text-xs text-muted">단가 {won(p.price)} · 최소 {p.moq}개</div>
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <button onClick={() => cart.remove(p.handle)} className="text-xs text-muted hover:text-ink">삭제</button>
                </div>
              </div>
      ...
      <aside className="card h-fit p-5 md:sticky md:top-32">
        <div className="flex justify-between text-sm"><span className="text-muted">공급가액</span><span>{won(subtotal)}</span></div>
        <div className="mt-2 flex justify-between text-sm"><span className="text-muted">부가세 (10%)</span><span>{won(vat)}</span></div>
        ...
        <div className="mt-5 grid gap-2">
          <Link href="/quote" className="btn-accent w-full">이 구성으로 견적요청</Link>
          {/* ponytail: 결제(토스/포트원)는 서버 연결 후 활성화 */}
          <button disabled className="btn-ghost w-full opacity-50" title="결제 연동 예정">바로 주문 (준비중)</button>
        </div>
        <p className="mt-3 text-[11px] leading-relaxed text-muted">대량 주문은 견적요청 시 제조사 협의 단가가 적용되어 표시 금액보다 낮아질 수 있습니다.</p>
      </aside>
    </div>
  );
}
